...
<div class="row mb">
    ...
    <div class="col-sm-8">
        ...
        <div class="btn-group">
            ...
            <ul class="dropdown-menu animated zoomIn">
                ...
                <?php } ?>
                <?php if ($estimates_info->status == 'Expired' || $estimates_info->status == 'Send' || $estimates_info->status == 'Cancelled') { ?>
                    <li>
                ...
                <?php } ?>
                <?php if ($estimates_info->status != 'Send' || $estimates_info->status == 'Expired') { ?>
                    <li>
                        <a href="<?= base_url() ?>admin/estimates/change_status/mark_as_sent/<?= $estimates_info->estimates_id ?>"
                           title="<?= lang('mark_as_sent') ?>"><?= lang('mark_as_sent') ?></a>
                    </li>
                <?php } ?>
                <?php if ($estimates_info->status == 'Pending' || $estimates_info->status == 'Send') { ?>
                    <li>
                ...
                <?php if ($estimates_info->status != 'Cancelled') { ?>
                    <li>
                        <a href="<?= base_url() ?>admin/estimates/change_status/mark_as_cancelled/<?= $estimates_info->estimates_id ?>"
                           title="<?= lang('mark_as_cancelled') ?>"><?= lang('mark_as_cancelled') ?></a>
                    </li>
                <?php } ?>
                <?php if ($estimates_info->status == 'Cancelled') { ?>
                    <li>
                        <a href="<?= base_url() ?>admin/estimates/change_status/unmark_as_cancelled/<?= $estimates_info->estimates_id ?>"
    ...
    <div class="col-sm-4 pull-right">
        ...
        <a data-toggle="tooltip" data-placement="top" title="<?= $title ?>"
           ...
           class="mr-sm btn pull-right  btn-sm  btn-<?= $btn ?>"><i class="fa fa-thumb-tack"></i></a>
    </div>
</div>
        ...
        <div class="row">
            <div class="col-xs-12 table-responsive">
                <form data-parsley-validate="" novalidate="" method="post"
                      action="<?= base_url() ?>admin/estimates/add_item/<?php
                      if (!empty($item_info)) {
                    ...
                    <table class="table">
                        ...
                        <tr class="hidden-print">
                            ...
                            <td><input type="text" data-parsley-type="number" name="quantity[]" value="<?php
                                if (!empty($item_info)) {
                            ...
                            <td><input type="text" data-parsley-type="number" name="unit_cost[]" value="<?php
                                if (!empty($item_info)) {
    ...
    $(document).ready(function () {
        ...
        $("#add_more").click(function () {
            if (maxAppend >= 4) {
                ...
                var add_new = $('<tr ><td class="col-sm-1"><input type="text" data-parsley-type="number" name="quantity[]" placeholder="1" required class="form-control"></td>">\n\
...
<?php
                    ...
                    if (!empty($tax_rates)) {
                    foreach ($tax_rates as $v_tax) {
                    ?><option value="<?= $v_tax->tax_rate_percent ?>"><?= $v_tax->tax_rate_name . ' (' . $v_tax->tax_rate_percent . '%)' ?></option><?php
                    }
                    }
                    ?></select></td>\n\
    <td class="col-sm-2"><input type="text" data-parsley-type="number" name="unit_cost[]" required placeholder="100" class="form-control"></td>\n\
