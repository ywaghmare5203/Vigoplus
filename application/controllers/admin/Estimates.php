...
class Estimates extends Admin_Controller
{
    ...
    public function save_estimates($id = NULL)
    {
        ...
        $data = $this->estimates_model->array_from_post(array('reference_no', 'client_id', 'project_id', 'tax', 'discount'));
    ...
    public function change_status($action, $id)
    {
        ...
        if (!empty($can_edit)) {
            ...
            if ($action == 'hide') {
                ...
                $data = array('status' => 'Pending');
            } elseif ($action == 'mark_as_sent') {
                $data = array('emailed' => 'Yes', 'date_sent' => date("Y-m-d H:i:s", time()), 'status' => 'Send');
            } else {
