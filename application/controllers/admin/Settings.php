...
class Settings extends Admin_Controller
{
    ...
    public function save_system()
    {
        ...
        $input_data = $this->settings_model->array_from_post(array('default_language', 'locale',
            ...
            'enable_languages', 'allow_client_registration', 'allow_client_project', 'currency_position', 'money_format', 'allowed_files',
            'auto_close_ticket', 'contract_expiration_reminder', 'attendance_report'));
    ...
    {
        $input_data = $this->settings_model->array_from_post(array('invoice_prefix', 'invoices_due_after', 'invoice_start_no', 'increment_invoice_number', 'show_invoice_tax', 'send_email_when_recur', 'default_terms', 'invoice_footer'));
        //image Process
...
// check existing mail 
    ...
    {
        $user_menu = $this->db->where('status', 1)->order_by('sort', 'time')->get('tbl_menu')->result();
        $menu = array(
            'items' => array(),
            'parents' => array()
        );
