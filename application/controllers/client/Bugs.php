...
// Update into tbl_project
    ...
    public function change_status($id, $status)
    {
        ...
        if (!empty($can_edit)) {
            $bugs_info = $this->bugs_model->check_by(array('bug_id' => $id), 'tbl_bug');
            $assigned_to = unserialize($bugs_info->assigned_to);
...
// send notification
    ...
    function notify_comments_bugs($comment_id)
    {
        ...
        $Link = str_replace("{COMMENT_URL}", base_url() . 'client/bugs/view_bug_details/' . $bugs_info->bug_id . '/' . $data['active'] = 2, $assigned_by);
        ...
        $params['message'] = $message;
        $params['resourceed_file'] = '';
        $users = unserialize($bugs_info->assigned_to);
