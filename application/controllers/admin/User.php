...
class User extends Admin_Controller
{
    ...
    public function user_details($id, $active = null)
    {
        ...
        $data['my_leave_report'] = $this->get_individual_leave_report($id);
        ...
        foreach ($all_category as $v_category) {
            ...
            if (!empty($all_user_report)) {
                foreach ($all_user_report as $v_user_report) {
                    $get_all_dates[$v_category->leave_category_id][] = $this->global_model->GetDays($v_user_report->leave_start_date, $v_user_report->leave_end_date);
                }
            }
        }
    ...
    {
        $bank_data = $this->user_model->array_from_post(array('bank_name', 'branch_name', 'account_name', 'account_number'));
    ...
    public function save_user()
    {
        ...
        if (!empty($check_user) || !empty($check_email)) { // if input data already exist show error alert
            ...
            $profile_data = $this->user_model->array_from_post(array('fullname', 'employment_id', 'company', 'locale', 'language', 'phone', 'mobile', 'skype', 'designations_id'));
    ...
    public function send_welcome_email($id)
    {
        ...
        $message = str_replace("{COMPANY_NAME}", config_item('company_name'), $NAME);
...
// crud for sidebar todo list
    ...
    public function reset_password($id)
    {
        ...
        if ($this->session->userdata('user_type') == 1) {
            ...
            $where = array('user_id' => $id);
            $action = array('password' => $this->user_model->hash("123456"));
            $this->user_model->set_action($where, $action, 'tbl_users');
            ...
            $this->send_email_reset_password($email, $user_info);
    ...
    function send_email_reset_password($email, $user_info)
    {
        ...
        $user_password = str_replace("{NEW_PASSWORD}", '123456', $user_email);
