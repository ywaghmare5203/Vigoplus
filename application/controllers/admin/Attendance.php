...
class Attendance extends Admin_Controller
{
    ...
    public function time_history()
    {
        ...
        $attendance_info = $this->attendance_model->get_by(array('user_id' => $data['user_id']), FALSE);
    ...
    {
        $cdata = $this->attendance_model->array_from_post(array('reason', 'clockin_edit', 'clockout_edit', 'datein_edit', 'dateout_edit'));
        ...
        //get attendance_id
        $result = $this->attendance_model->selectclock($history_id);
        //update date here
        $d['date_in'] = $cdata['datein_edit'];
        $d['date_out'] = $cdata['dateout_edit'];
        $this->attendance_model->_table_name = "tbl_attendance";
        $this->attendance_model->_primary_key = "attendance_id";
        $re = $this->attendance_model->save($d, $result->attendance_id);
    ...
    public function saved_manual_time()
    {
        ...
        $hdata['reason'] = lang('manually_added');
        $hdata['status'] = '2';

        $this->attendance_model->_table_name = "tbl_clock_history"; // table name
        $this->attendance_model->_primary_key = "clock_history_id"; // $id
        $this->attendance_model->save($hdata);
    ...
    public function get_report_2()
    {
        ...
        foreach ($data['employee'] as $sl => $v_employee) {
            ...
            for ($i = 1; $i <= $num; $i++) {
                ...
                if (!empty($flag)) {
                    $data['attendance'][$sl][] = $this->attendance_model->attendance_report_by_empid($v_employee->user_id, $sdate, $flag);
                } else {
                    $data['attendance'][$sl][] = $this->attendance_model->attendance_report_by_empid($v_employee->user_id, $sdate);
                }
