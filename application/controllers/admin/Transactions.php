...
class Transactions extends Admin_Controller
{
    ...
    {
        $data = $this->transactions_model->array_from_post(array('date', 'notes', 'category_id', 'paid_by', 'payment_methods_id', 'reference', 'project_id'));

        $data['type'] = 'Expense';

        $client_visible = $this->input->post('client_visible', TRUE);
