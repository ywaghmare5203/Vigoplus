...
class Tickets extends Client_Controller
{
    ...
    function get_notify_ticket_reply($users, $postdata)
    {
        ...
        $designations_info = $this->db->where('departments_id', $tickets_info->departments_id)->get('tbl_designations')->result();


        switch ($users) {
            case 'admin':
                if (!empty($designations_info)) {
                    foreach ($designations_info as $v_designation) {
                        $user_info = $this->db->where('designations_id', $v_designation->designations_id)->get('tbl_account_details')->result();
                        if (!empty($user_info)) {
                            foreach ($user_info as $v_user) {
                                $login_info = $this->tickets_model->check_by(array('user_id' => $v_user->user_id), 'tbl_users');
                                $params['recipient'] = $login_info->email;
