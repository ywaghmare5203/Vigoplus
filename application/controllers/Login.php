...
class Login extends MY_Controller {
    ...
    public function forgot_password() {
        ...
        if (!empty($flag)) {
            ...
            if (!empty($login_details)) {
                ...
                $type = 'success';
                $message = lang('message_new_password_confir');
    ...
    public function reset_password($user_id, $new_pass_key) {
        ...
        $check_reset_pass = $this->login_model->can_reset_password($user_id, $new_pass_key, $this->config->item('forgot_password_expire', 'login'));

        if ($check_reset_pass == true) {
            $this->login_model->get_reset_password($user_id, $new_pass_key);
    ...
    public function registered_user() {
        ...
        if (!empty($check_email) || !empty($check_username)) {
            ...
            $client = $this->login_model->array_from_post(array('client_status', 'name', 'email'));
