...
class MY_Model extends CI_Model
{
    ...
    public function selectclock($historyId)
    {
        $this->db->select('tbl_clock.*',FALSE);
        $this->db->select('tbl_clock_history.*',FALSE);
        $this->db->from('tbl_clock_history');
        $this->db->join('tbl_clock','tbl_clock.clock_id = tbl_clock_history.clock_id');
        $this->db->where('clock_history_id',$historyId);
...
//        $config['max_height'] = '768';
   ...
   /* public function all_permission_user($menu_id)
    ...
    {
        $query = $this->db->select('reference_no')->select_max('invoices_id')->get('tbl_invoices');
        if ($query->num_rows() > 0) {
            $row = $query->row();
    ...
    {
        $query = $this->db->select('reference_no')->select_max('estimates_id')->get('tbl_estimates');
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $ref_number = intval(substr($row->reference_no, -4));
    ...
    public
    function extimate_reference_no_exists($next_number)
    {
        $next_number = sprintf('%04d', $next_number);
...
$this->email->from('sales@divinewelfare.com', 'Your Name');
//$this->email->to('devendra.sharma@consagous.com');
