...
//            $subview = 'payments_pdf';
    ...
    public function save_invoice($id = NULL)
    {
        ...
        $data = $this->invoice_model->array_from_post(array('reference_no', 'client_id', 'project_id', 'tax', 'discount'));
        ...
        if (!empty($quantity)) {
            foreach ($quantity as $key => $value) {
                if (!empty($value)) {
                    ...
                    if (!empty($id)) {
                        $items_id = $id;
                        $this->invoice_model->save($q_data, $id);
                    } else {
                        $items_id = $this->invoice_model->save($q_data);
                    }
                }
            }
        }
        ...
        if (!empty($can_edit)) {
            ...
            if ($paid_amount != 0) {
                if ($paid_amount > $due) {
                    ...
                    if ($this->input->post('save_into_default_account') == 'on') {
    ...
    public function change_invoice_status($action, $id)
    {
