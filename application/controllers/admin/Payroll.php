...
// ************* Save into tbl_salary_template *************
        ...
        if (!empty($id)) {
            if (empty($medical_allowance)) {
                $asalary_allowance_data['allowance_label'][] = lang('medical_allowance');
                $asalary_allowance_data['allowance_value'][] = 0;
            }
            if (!empty($house_rent_allowance)) {
                $asalary_allowance_data['allowance_label'][] = lang('house_rent_allowance');
                $asalary_allowance_data['allowance_value'][] = 0;
...
// get salary deduction info by salary template id
        ...
        pdf_create($viewfile, lang('salary_template') . '-' . $data['salary_template_info']->salary_grade);
    }
...
// get salary deduction info by salary template id
        ...
        $this->load->helper('dompdf');
        pdf_create($viewfile, lang('salary_details') . '- ' . $data['emp_salary_info']->fullname);
    }
    ...
    public function save_advance_salary($id = null)
    {
        ...
        $salary_info = $this->payroll_model->check_by(array('advance_salary_id' => $id), 'tbl_advance_salary');
        $profile_info = $this->payroll_model->check_by(array('user_id' => $salary_info->user_id), 'tbl_account_details');
        // send email to departments head
        if ($salary_info->status == 1) {
            ...
            if (!empty($dept_head->department_head_id)) {
                ...
                if (!empty($advance_salary_email) && $advance_salary_email == 1) {
                    ...
                    $Link = str_replace("{LINK}", base_url() . 'admin/payroll/advance_salary_details/' . $id, $username);
    ...
    public function advance_salary_pdf($year, $month)
    {
        ...
        pdf_create($viewfile, lang('advance_salary_report') . ' - ' . $data['monthyaer']);
    }

    public function advance_salary_details($id)
    {
        $data['title'] = lang('advance_salary_details');
        $data['advance_salary_info'] = $this->payroll_model->get_advance_salary_info_by_date('', $id);
    ...
    public function set_salary_status($status, $id)
    {
        ...
        if ($salary_info->status == '0') {
            $status = lang('pending');
        } elseif ($salary_info->status == '1') {
            $status = lang('accepted');
            $this->send_email_by_status($salary_info, true);
        } elseif ($salary_info->status == '2') {
            $status = lang('rejected');
            $this->send_email_by_status($salary_info);
        } else {
            $status = lang('paid');
        }
        // save into activities
        $activities = array(
            ...
            'icon' => 'fa-ticket',
            'value1' => $profile_info->fullname . ' ' . lang('request_date') . strftime(config_item('date_format'), strtotime($salary_info->request_date)),
            'value2' => $status . ' ' . lang('amount') . ': ' . $salary_info->advance_amount,
        );
        // Update into tbl_project
        $this->payroll_model->_table_name = "tbl_activities"; //table name
        $this->payroll_model->_primary_key = "activities_id";
    ...
    function send_email_by_status($salary_info, $approve = null)
    {
        $advance_salary_email = config_item('advance_salary_email');
        if (!empty($advance_salary_email) && $advance_salary_email == 1) {
            ...
            $user_info = $this->payroll_model->check_by(array('user_id' => $salary_info->user_id), 'tbl_users');
            $curency = $this->db->where('code', config_item('default_currency'))->get('tbl_currencies')->row();
            $message = $email_template->template_body;
            $subject = $email_template->subject;
            $advance_amount = str_replace("{AMOUNT}", display_money($salary_info->advance_amount, $curency->symbol), $message);
        ...
        if ($user_id != 0) {
            if (!empty($payment_month)) {
                ...
                if (!empty($data['total_hours'])) {
                    foreach ($data['total_hours'] as $v_hours) {
                        if ($v_hours['total_hours'] == 0 && $v_hours['total_minutes'] == 0) {
                            $type = 'error';
                ...
                if (!empty($designation_info)) {
                    foreach ($designation_info as $v_designatio) {
                        ...
                        foreach ($employee_info as $value) {
                            ...
                            $data['salary_info'][] = $this->payroll_model->check_by(array('user_id' => $value->user_id, 'payment_month' => $data['payment_month']), 'tbl_salary_payment');
    ...
    public function get_advance_salary_info_by_id($user_id, $payment_month)
    {
        ...
        foreach ($advance_salary_info as $v_advance_salary) {
            $result['advance_amount'] = $v_advance_salary->advance_amount;
        }
        if (!empty($result)) {
            return $result;
        }
...
//        // save into tbl employee paymenet
        ...
        if (!empty($employee_info->salary_template_id)) {
            ...
            if (!empty($employee_info->overtime_salary)) {
                $salary_payment_details_label[] = lang('overtime_salary') . ' <small>( ' . lang('per_hour') . ')</small>';
            ...
            if ($overtime_minutes > 0) {
                $ov_amount = 60 / $employee_info->overtime_salary;
        ...
        if (!empty($employee_info->hourly_rate_id)) {
            ...
            $salary_payment_details_label[] = lang('hourly_rates');
        ...
        if (!empty($award_info)) {
            foreach ($award_info as $v_award_info) {
                $salary_payment_details_label[] = lang('award_name') . '<small> ( ' . $v_award_info->award_name . ' )</small>';
        ...
        if (!empty($employee_info->hourly_rate_id) || !empty($employee_info->salary_template_id)) {
            ...
            $r_data['account_id'] = config_item('default_account');
            if (!empty($r_data['account_id'])) {
                $account_id = $r_data['account_id'];
            } else {
    ...
    public function salary_payment_details_pdf($salary_payment_id)
    {
        ...
        $this->load->helper('dompdf');
        pdf_create($viewfile, lang('salary_details') . '- ' . $data['salary_payment_info']->fullname);
    }
