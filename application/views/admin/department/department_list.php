        ...
        dataSource: [
            {
                ...
                <?php foreach ($v_parent as $parent_id => $v_child){ ?>
                {
                    ...
                    <?php foreach ($v_sub_child as $sub_chld => $v_sub_chld){ ?>
                    {
                        ...
                        <?php if (is_array($sub_chld_id)){ ?>
                        <?php foreach ($sub_chld_id as $sub_chld_1 => $v_sub_chld_1){ ?>
                        {
                            id: "", text: "<?php echo lang($sub_chld_name); ?>", value: "<?php
                            if (!empty($sub_chld_1)) {
                                echo $sub_chld_1;
                            }
                            ?>", expanded: false, items: [
                            <?php foreach ($v_sub_chld_1 as $sub_chld_name_1 => $v_sub_chld_2){ ?>
                            <?php if (is_array($v_sub_chld_2)){ ?>
                            {
                                <?php foreach ($v_sub_chld_2 as $sub_chld_name_2 => $v_sub_chld_3){ ?>
                                id: "", text: "<?php echo lang($sub_chld_name_1); ?>", value: "<?php
                                if (!empty($sub_chld_name_2)) {
                                    echo $sub_chld_name_2;
                                }
                                ?>", expanded: false, items: [
                                <?php foreach ($v_sub_chld_3 as $sub_chld_name_3 => $v_sub_chld_4){ ?>
                                <?php if (is_array($v_sub_chld_4)){ ?>

                                <?php }else{?>
                                {
                                    id: "", text: "<?php echo lang($sub_chld_name_3); ?>",<?php
                                    if (!is_array($v_sub_chld_4)) {
                                        if (!empty($roll[$v_sub_chld_4])) {
                                            echo $roll[$v_sub_chld_4] ? 'check: "checked",' : '';
                                        }
                                    }
                                    ?> value: "<?php
                                    if (!empty($v_sub_chld_4)) {
                                        echo $v_sub_chld_4;
                                    }
                                    ?>",
                                },
                            ...
                            <?php }else{?>
                            {
                                id: "", text: "<?php echo lang($sub_chld_name_1); ?>",<?php
                                if (!is_array($v_sub_chld_2)) {
                                    if (!empty($roll[$v_sub_chld_2])) {
                                        echo $roll[$v_sub_chld_2] ? 'check: "checked",' : '';
                                    }
                                }
                                ?> value: "<?php
                                if (!empty($v_sub_chld_2)) {
                                    echo $v_sub_chld_2;
                                }
                                ?>",
                            },
